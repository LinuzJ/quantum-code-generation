...
def load_quasar_ground_truth(quasar_path: str, simulator: AerSimulator) -> Tuple[List[Optional[List[float]]], List[Optional[int]]]:
	...
	return solution_probs, solution_qubits


def compare_circuit_to_solution(
	gen_qasm: str, solution_probs: List[float], simulator: AerSimulator
) -> Tuple[Optional[float], Optional[int]]:
	"""Compare generated circuit to solution and return KL divergence and qubit count."""
	try:
		...
		return float(kl), circ.num_qubits
	except Exception:
		return None, None
...
def main():
	...
	solution_probs_list, solution_qubits_list = load_quasar_ground_truth(quasar_path, simulator)
	...
	# Initialize counts: model -> qubit_count -> (success_count, total_count)
	...
	for i in range(denominator):
		...
		if solution_probs is None or solution_qubits is None:
			continue
		...
		for label, cfg in models.items():
			...
			if kind == "folder":
				...
				if not content:
					continue
				kl, num_qubits = compare_circuit_to_solution(content, solution_probs, simulator)
				if kl is None or num_qubits is None:
					continue
				# Track by the ground truth qubit count (not the generated circuit's qubit count)
				counts[label][solution_qubits][1] += 1  # total count
				if kl < threshold:
					counts[label][solution_qubits][0] += 1  # success count
	...
	for i in range(min(denominator, len(quasar_data))):
		...
		try:
			...
			continue
		kl, num_qubits = compare_circuit_to_solution(response_qasm, solution_probs, simulator)
		if kl is None or num_qubits is None:
			continue
